    ...
    public class ReportesServices : BaseService
    {
        ...
        public IEnumerable<ReportePrestamo> ObtenerPago(int ID_PAGO)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                int ID_PRESTAMO = pago.ID_PRESTAMO;
                ...
                foreach (var item in kardex)
                {
                    ...
                    kar.DEBE = planPago.SALDO_PRESTAMO;
                    result.Add(kar);

                    var kar2 = new ReportePrestamo()
                    {
                        ID_PAGO = item.ID_PAGO,
                        ID_PRESTAMO = item.ID_PRESTAMO,
                        FECHA_PAGO = item.FECHA,
                    ...
                    if (item.SD_PRESTAMOS_MORA != null)
                    {
                        ...
                        {
                            ID_PAGO = item.ID_PAGO,
                            ID_PRESTAMO = (decimal)item.ID_PRESTAMO,
                            FECHA_PAGO = item.FECHA,
                            FECHA_LIMITE_PAGO = item.SD_PRESTAMOS_POR_SOCIOS.FECHA_LIMITE_PAGO,
                        ...
                        karMora.DEBE = planPago.SALDO_PRESTAMO;
                        result.Add(karMora);
                    }
                }
                //FE
        ...
        public IEnumerable<ReporteDetalleIngresos> ObtenerReporteIngresoDetalle(string CONDICION, DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                var otrosingresos = managerOtrosIngresos.BuscarTodos(x => x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin && x.ESTADO != "ANULADO");
                ...
                foreach (var item1 in ingresosDeudas)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        ...
                        DOLARES = item1.SD_CAJAS.MONEDA == "DOLARES" ? (decimal)item1.IMPORTE_CANCELADO : 0
                    };
                    result.Add(detalleRep);
                }

                var managerRegularizaciones = new SD_DETALLES_REGULARIZACIONESManager(uow);
                var ingresosRegularizaciones = managerRegularizaciones.BuscarTodos(x => x.SD_REGULARIZACIONES.FECHA_COMPRA >= FECHA_INI && x.SD_REGULARIZACIONES.FECHA_COMPRA < Fecha_fin && x.SD_REGULARIZACIONES.ESTADO != "ANULADO");
                foreach (var item1 in ingresosRegularizaciones)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        DETALLE = "OTROS INGRESOS (REGULARIZACIONES)",
                        SUBDETALLE = item1.SD_REGULARIZACIONES.SD_SOCIO_MOVILES.ObtenerNombreSocio() + " " + item1.SD_REGULARIZACIONES.MES.ToString("MMMM - yyyy"),
                        FECHA = (DateTime)item1.SD_REGULARIZACIONES.FECHA_COMPRA,
                        FECHA_INI = FECHA_INI,
                        FECHA_FIN = FECHA_FIN,
                        NRO_MOVIL = item1.SD_REGULARIZACIONES.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL,
                        NRO_RECIBO = Convert.ToString(item1.ID_REGULACION),
                        CAJA = item1.SD_CAJAS.NOMBRE,
                        BOLIVIANOS = item1.SD_CAJAS.MONEDA == "BOLIVIANOS" ? (decimal)item1.IMPORTE : 0,
                        DOLARES = item1.SD_CAJAS.MONEDA == "DOLARES" ? (decimal)item1.IMPORTE : 0
                    };
                    result.Add(detalleRep);
                }
                //FE
        ...
        public IEnumerable<ReporteDetalleIngresos> ObtenerReporteIngresoPorCategoria(string CATEGORIA, DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            List<ReporteDetalleIngresos> result = new List<ReporteDetalleIngresos>();
            List<ReporteDetalleIngresos> result1 = new List<ReporteDetalleIngresos>();
            NumLetra n = new NumLetra();
            ExecuteManager(uow =>
            {
                ...
                if (CATEGORIA.Trim() == "ITEMS INGRESOS EN BS") {
                    ...
                    foreach (var item in hojas)
                    {
                        ...
                        var detalleRep = new ReporteDetalleIngresos()
                        {
                            ...
                            NRO_RECIBO = string.Format("{0} - {1}", item.ID_HOJA, item.NRO_HOJA),
                            CAJA = item.SD_DETALLES_HOJAS_CONTROL.FirstOrDefault().SD_CAJAS.NOMBRE,
                            BOLIVIANOS = item.SD_DETALLES_HOJAS_CONTROL.FirstOrDefault().SD_CAJAS.MONEDA == "BOLIVIANOS" ? (decimal)item.MONTO : 0,
                            DOLARES = item.SD_DETALLES_HOJAS_CONTROL.FirstOrDefault().SD_CAJAS.MONEDA == "DOLARES" ? (decimal)item.MONTO : 0
                        };
                        result.Add(detalleRep);
                    }
                }
                ...
                var otrosingresosDeudas = managerDuedas.BuscarTodos(x => x.FECHA_CANCELADO >= FECHA_INI && x.FECHA_CANCELADO < Fecha_fin && x.ESTADO != "ANULADO" && x.SD_DEUDAS_SOCIOS.CATEGORIA == CATEGORIA);
                foreach (var item1 in otrosingresosDeudas)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        DETALLE = item1.SD_DEUDAS_SOCIOS.MOTIVO,
                        SUBDETALLE = item1.SD_SOCIO_MOVILES.ObtenerNombreSocio(),
                        FECHA = (DateTime)item1.FECHA_CANCELADO,
                        FECHA_INI = FECHA_INI,
                        FECHA_FIN = FECHA_FIN,
                        NRO_MOVIL = item1.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL,
                        NRO_RECIBO = Convert.ToString(item1.ID_DETALLE),
                        CAJA = item1.SD_CAJAS.NOMBRE,
                        BOLIVIANOS = item1.SD_DEUDAS_SOCIOS.SD_CAJAS.MONEDA == "BOLIVIANOS" ? (decimal)item1.IMPORTE_CANCELADO : 0,
                        DOLARES = item1.SD_DEUDAS_SOCIOS.SD_CAJAS.MONEDA == "DOLARES" ? (decimal)item1.IMPORTE_CANCELADO : 0
                    };
                    result.Add(detalleRep);
                }
            });
        ...
        public IEnumerable<ReporteDetalleIngresos> ObtenerReporteEgresosPorCategoria(string CATEGORIA,DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                DateTime Fecha_fin = FECHA_FIN.AddDays(1);
                var managerIngresos = new SD_EGRESOSManager(uow);
                var ingresosSocios = managerIngresos.BuscarTodos(x => x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin && x.ESTADO != "ANULADO" && x.SD_TIPOS_EGRESOS.CATEGORIA == CATEGORIA);
                foreach (var item in ingresosSocios)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
        ...
        public IEnumerable<ReportePrestamo> ObtenerReporteTotalPrestamos(DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                decimal? transferenciasIngreso = managerTransferencias.BuscarTodos(x => x.ID_CAJA_DESTINO == ID_CAJA_PRESTAMO && x.ESTADO != "ANULADO").Sum(y => (decimal?)y.IMPORTE);
                decimal? transferenciasEgreso = managerTransferencias.BuscarTodos(x => x.ID_CAJA_ORIGEN == ID_CAJA_PRESTAMO && x.ESTADO != "ANULADO").Sum(y => (decimal?)y.IMPORTE);

                decimal? amortizazcion = managerAmortizaciones.BuscarTodos(x => x.ID_CAJA == ID_CAJA_PRESTAMO && x.ESTADO != "ANULADO").Sum(y => (decimal?)y.IMPORTE);
                ...
                foreach (var item in ingresosSocios)
                {
                    var moras = managerMoras.BuscarTodos(x => x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin && x.SD_PRESTAMOS_POR_SOCIOS.ID_TIPO_PRESTAMO == item.ID_TIPO_PRESTAMO);
                    var pagos = managerPagos.BuscarTodos(x => x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin && x.SD_PRESTAMOS_POR_SOCIOS.ID_TIPO_PRESTAMO == item.ID_TIPO_PRESTAMO && x.ESTADO != "ANULADO");
                    ...
                    foreach (var item1 in pagos.GroupBy(y=>y.ID_PRESTAMO))
                    {
                        ...
                        decimal inte = managerIngresos.BuscarTodos(x => x.ID_PRESTAMO == item1.Key).Sum(y => y.SD_PLAN_DE_PAGO.Where(xx => xx.ESTADO == "CANCELADO").Sum(xy => xy.INTERES_A_PAGAR));
                        interesCancelada = interesCancelada + inte;
                        decimal cond = managerIngresos.BuscarTodos(x => x.ID_PRESTAMO == item1.Key).Sum(y => y.SD_PLAN_DE_PAGO.Where(xx => xx.ESTADO == "CANCELADO").Sum(xy => xy.CONDONACION));
                        condonacionCan = condonacionCan + cond;

                        decimal mor = managerIngresos.BuscarTodos(x => x.ID_PRESTAMO == item1.Key).Sum(y => y.SD_PLAN_DE_PAGO.Where(xx => xx.ESTADO == "CANCELADO").Sum(xy => xy.MORA_A_PAGAR));
                        moraCancelada = moraCancelada + mor;
                    }
                    ...
                    var detalleRep = new ReportePrestamo()
                    {
                        ...
                        CAPITAL_CANCELADO = capitalCancelada,
                        INTERES_CANCELADO = interesCancelada,
                        MORA_CANCELADA = moraCancelada,
        ...
        public IEnumerable<ReportePrestamo> ObtenerReporteTotalPrestamosV2(DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            List<ReportePrestamo> result = new List<ReportePrestamo>();
            NumLetra n = new NumLetra();
            ExecuteManager(uow =>
            {
                ...
                foreach (var item in ingresosSocios.OrderBy(x=>x.ID_PRESTAMO))
                {
                    ...
                    foreach (var item1 in pagos)
                    {
                        ...
                        interesCancelada = interesCancelada + inte;
                        decimal cond = managerPlanPagos.BuscarTodos(x => x.ID_PRESTAMO == item1.ID_PRESTAMO && idPago == x.ID_PAGO && x.ESTADO == "CANCELADO").Sum(y => y.CONDONACION);
                        condonacionCan = condonacionCan + cond;

                        decimal mor = managerPlanPagos.BuscarTodos(x => x.ID_PRESTAMO == item1.ID_PRESTAMO && idPago == x.ID_PAGO && x.ESTADO == "CANCELADO").Sum(y => y.MORA_A_PAGAR);
                        moraCancelada = moraCancelada + mor;
                    }
        ...
        public IEnumerable<EstadoResultadoCompletoModel> ReporteEstadoResultadoPorCaja(int ID_CAJA, DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                var managerTransferencias = new SD_TRANSFERENCIASManager(uow);
                var managerDeudas = new SD_DETALLES_DEUDASManager(uow);
                int cnt = 0;
                ...
                var detallesPagosDeudas = managerDeudas.BuscarTodos(x => x.ID_CAJA == ID_CAJA && x.FECHA_CANCELADO >= FECHA_INI && x.FECHA_CANCELADO < Fecha_fin && x.IMPORTE_CANCELADO > 0).GroupBy(y => y.SD_DEUDAS_SOCIOS.MOTIVO);
                if (detallesPagosDeudas.Count() > 0)
                {
                    foreach (var item in detallesPagosDeudas)
                    {
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            NRO = cnt,
                            OPERACION = "INGRESOS",
                            SUBOPERACION = "PAGO DE DEUDAS",
                            DETALLE = item.Key,
                            PERIODO = Intervalo,
                            CAJA = caja,
                            MONEDA = moneda,
                            //MES =  item.FECHA.ToString("MMMM").ToUpper();
                            IMPORTE = item.Sum(x => x.IMPORTE_CANCELADO),
                            //UTILIDA_BRUTA_NETA = (decimal)(totalventa - totalcosto)
                        };
                        result.Add(res);
                        cnt++;
        ...
        public IEnumerable<EstadoResultadoCompletoModel> ReporteEstadoResultadoDetallePorCaja(int ID_CAJA, DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            List<EstadoResultadoCompletoModel> result = new List<EstadoResultadoCompletoModel>();
            ...
            ExecuteManager(uow =>
            {
                ...
                if (detalleshojas.Count() > 0)
                {
                    foreach (var item in detalleshojas)
                    {
                        ...
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            ...
                            OPERACION = "INGRESOS",
                            SUBOPERACION = "VENTA DE HOJAS",
                            DETALLE = string.Format("({0})  {1} NRO MOVIL : {2}", item.SD_HOJAS_CONTROL.FECHA_COMPRA.ToString("dd/MM/yyyy"), item.OBLIGACION, item.SD_HOJAS_CONTROL.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL),
                            PERIODO = Intervalo,
                            CAJA = caja,
                            MONEDA = moneda,
                            //MES =  item.FECHA.ToString("MMMM").ToUpper();
                ...
                if (detalleregularizaciones.Count() > 0)
                {
                    foreach (var item in detalleregularizaciones)
                    {
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            ...
                            OPERACION = "INGRESOS",
                            SUBOPERACION = "REGULARIZACIONES",
                            DETALLE = string.Format("({0})  {1} NRO MOVIL : {2}", item.SD_REGULARIZACIONES.FECHA_COMPRA.Value.ToString("dd/MM/yyyy"), item.OBLIGACION, item.SD_REGULARIZACIONES.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL),
                            PERIODO = Intervalo,
                            CAJA = caja,
                            MONEDA = moneda,
                            IMPORTE = item.IMPORTE,
                ...
                if (detallesIngresosPorSocios.Count() > 0)
                {
                    ...
                    foreach (var item in detallesIngresosPorSocios)
                    {
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            NRO = cnt,
                            OPERACION = "INGRESOS",
                            SUBOPERACION = item.SD_TIPOS_INGRESOS_SOCIO.NOMBRE,
                            DETALLE = string.Format("({0})   NRO MOVIL : {1}", item.FECHA.ToString("dd/MM/yyyy"), item.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL),
                            PERIODO = Intervalo,
                            CAJA = caja,
                            MONEDA = moneda,
                            //MES =  item.FECHA.ToString("MMMM").ToUpper();
                ...
                if (detallesIngresos.Count() > 0)
                {
                    foreach (var item in detallesIngresos)
                    {
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            NRO = cnt,
                            OPERACION = "INGRESOS",
                            SUBOPERACION = item.TIPO,
                            DETALLE = string.Format("({0}) {1} ", item.FECHA.ToString("dd/MM/yyyy"), item.CONCEPTO),
                            PERIODO = Intervalo,
                            MONEDA = moneda,
                            //MES =  item.FECHA.ToString("MMMM").ToUpper();
                            IMPORTE = item.IMPORTE
                ...
                var detallesPagosPrestamos = managerPagosPrestamos.BuscarTodos(x => x.ID_CAJA == ID_CAJA && x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin && x.ESTADO != "ANULADO");
                if (detallesPagosPrestamos.Count() > 0)
                {
                    foreach (var item in detallesPagosPrestamos)
                    {
                ...
                var detallesPagosDeudas = managerDeudas.BuscarTodos(x => x.ID_CAJA == ID_CAJA && x.FECHA_CANCELADO >= FECHA_INI && x.FECHA_CANCELADO < Fecha_fin && x.IMPORTE_CANCELADO > 0);
                if (detallesPagosDeudas.Count() > 0)
                {
                    foreach (var item in detallesPagosDeudas)
                    {
                        EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                        {
                            ...
                            OPERACION = "INGRESOS",
                            SUBOPERACION = "PAGO DE DEUDAS",
                            DETALLE = string.Format("({0}) {1} NRO MOVIL:{2}", item.FECHA_CANCELADO.Value.ToString("dd/MM/yyyy"), item.SD_DEUDAS_SOCIOS.MOTIVO, item.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL),
                            PERIODO = Intervalo,
                            CAJA = caja,
                            MONEDA = moneda,
                            //MES =  item.FECHA.ToString("MMMM").ToUpper();
                            IMPORTE = item.IMPORTE_CANCELADO,
                            //UTILIDA_BRUTA_NETA = (decimal)(totalventa - totalcosto)
                        };
                        result.Add(res);
                        cnt++;
                ...
                var detallePrestamos = managerPrestamos.BuscarTodos(x => x.ID_CAJA == ID_CAJA && x.ESTADO != "ANULADO" && x.FECHA >= FECHA_INI && x.FECHA < Fecha_fin);
                foreach (var item in detallePrestamos)
                {
                    EstadoResultadoCompletoModel res = new EstadoResultadoCompletoModel()
                    {
                        ...
                        OPERACION = "EGRESOS",
                        SUBOPERACION = "PRESTAMOS",
                        DETALLE = string.Format("({0}) {1} NRO MOVIL:{2}", item.FECHA.ToString("dd/MM/yyyy"), item.SD_TIPOS_PRESTAMOS.NOMBRE, item.SD_SOCIO_MOVILES.SD_MOVILES.NRO_MOVIL),
                        PERIODO = Intervalo,
                        CAJA = caja,
                        MONEDA = moneda,
                        //MES =  item.FECHA.ToString("MMMM").ToUpper();
                        IMPORTE = -item.IMPORTE_PRESTAMO,
                        //UTILIDA_BRUTA_NETA = (decimal)(totalventa - totalcosto)
                    };
                    result.Add(res);
                    cnt++;
        ...
        public IEnumerable<EstadoResultadoCompletoModel> ReporteEstadoResultadoPorMoneda(string MONEDA, DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                var detallesPagosDeudas = managerDeudas.BuscarTodos(x => x.SD_CAJAS.MONEDA == MONEDA && x.FECHA_CANCELADO >= FECHA_INI && x.FECHA_CANCELADO < Fecha_fin && x.IMPORTE_CANCELADO > 0).GroupBy(y => y.SD_DEUDAS_SOCIOS.MOTIVO);
                if (detallesPagosDeudas.Count() > 0)
                {
                    foreach (var item in detallesPagosDeudas)
                    {
        ...
        public IEnumerable<ReporteDetalleIngresos> ObtenerReporteTotalIngresosCategorias( DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            ...
            ExecuteManager(uow =>
            {
                ...
                foreach (var item in ingresosSocios)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        DETALLE = item.SD_TIPOS_INGRESOS_SOCIO.CATEGORIA,
                        SUBDETALLE = string.Format("{0} ({1})", item.OBSERVACION, item.SD_SOCIO_MOVILES.ObtenerNombreSocio()),
                        FECHA = item.FECHA,
                        FECHA_INI = FECHA_INI,
                        FECHA_FIN = FECHA_FIN,
                ...
                foreach (var item1 in ingresosDeudas)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        DETALLE = item1.SD_DEUDAS_SOCIOS.CATEGORIA,
                        SUBDETALLE = string.Format("{0} ({1})", item1.SD_DEUDAS_SOCIOS.MOTIVO, item1.SD_SOCIO_MOVILES.ObtenerNombreSocio()),
                        FECHA = (DateTime)item1.FECHA_CANCELADO,
                        FECHA_INI = FECHA_INI,
                        FECHA_FIN = FECHA_FIN,
        ...
        public IEnumerable<ReporteDetalleIngresos> ObtenerReporteTotalEgresosCategorias(DateTime FECHA_INI, DateTime FECHA_FIN)
        {
            List<ReporteDetalleIngresos> result = new List<ReporteDetalleIngresos>();
            NumLetra n = new NumLetra();
            ExecuteManager(uow =>
            {
                ...
                foreach (var item in ingresosSocios)
                {
                    var detalleRep = new ReporteDetalleIngresos()
                    {
                        DETALLE = item.SD_TIPOS_EGRESOS.CATEGORIA,
                        FECHA = item.FECHA,
                        FECHA_INI = FECHA_INI,
                        FECHA_FIN = FECHA_FIN,
                        CAJA = item.SD_CAJAS.NOMBRE,
