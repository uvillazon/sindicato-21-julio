Ext.define("App.View.Utils.Principal", {
    ...
    CargarComponentesUtils : function(){
        ...
        me.txt_controlador = Ext.create("App.Config.Componente.TextFieldBase", {
            name: 'CONTROLADOR',
            width : 300, 
            fieldLabel : 'controlador'
        });
        me.txt_tabla = Ext.create("App.Config.Componente.TextFieldBase", {
            name: 'TABLA',
            width : 350,
            fieldLabel : 'Tabla'
        });
...
//            items : me.editarStore
        ...
        me.store = Ext.create('App.Store.Utils.Tablas');
                ...
                '<p>Ext.define("App.controlador.Model.modelo",{</p>',
                 ...
                 '<tpl for=".">',
                  '<tpl for="data">',
                        ...
                        '<tpl if="TIPO == \'DATE\'"><p>{ type: "date", name: "{COLUMNA}",  dateFormat: "d/m/Y", convert: Funciones.Fecha },</p></tpl>',
                  '</tpl>',
                '</tpl>',
                '<p>]</p>',
                '<p>});</p>',
                 ...
                 '<tpl for=".">',
                  '<tpl for="data">',
                       '<tpl if="this.esLista(COMENTARIO,COLUMNA)">',
                            '<tpl if="COLUMNA.substr(0,2) == \'ID\'"> <p>{ header: " - ", width: 30, sortable: false, dataIndex: "{COLUMNA}", hidden : true },</p>',
                            '<tpl elseif="COLUMNA.substr(0,5) == \'FECHA\'">',
                                '<p>{ header: "{COLUMNA}", width: 70, sortable: true, dataIndex: "{COLUMNA}" ,renderer: Ext.util.Format.dateRenderer(\'d/m/Y\') },</p>',
                ...
                '<tpl switch="TIPO">',
                    '<tpl case="NUMBER">',
                        '<tpl if="COLUMNA.substr(0,2) == \'ID\'">',
                            '<p>me.txt_id = Ext.create("App.Config.Componente.TextFieldBase", &#123;</p>',
                            '<p> name: "{COLUMNA}",</p>',
                            '<p> hidden: true,</p>',
                            '<p> &#125;);</p>',
                        '<tpl elseif="COLUMNA.substr(0,4) == \'ANIO\'">',
                            '<p>me.num_{[values.COLUMNA.toLowerCase()]} = Ext.create("App.Config.Componente.NumberFieldBase", &#123;</p>',
                            ...
                            '<tpl if="REQUERIDO == \'N\'">',
                                '<p> afterLabelTextTpl: Constantes.REQUERIDO,</p>',
                                '<p> allowBlank: false,</p>',
                            '</tpl>',
                            '<p> &#125;);</p>',
                        '<tpl elseif="this.esLista(COMENTARIO)">',
                            '<p>me.cbx_{[values.COLUMNA.toLowerCase()]} = Ext.create("App.Config.Componente.ComboBase", &#123;</p>',
                            '<p> fieldLabel: "{COLUMNA}",</p>',
                            '<p> name: "{COLUMNA}",</p>',
                            '<p> width: 240,</p>',
                     ...
                     '<tpl case="VARCHAR2">',
                         '<tpl if="this.esLista(COMENTARIO)">',
                            ...
                            '<p> &#125;);</p>',
                         '<tpl else>',
                            '<p> me.txt_{[values.COLUMNA.toLowerCase()]} = Ext.create("App.Config.Componente.TextFieldBase", &#123; </p>',
                            '<p> fieldLabel: "{COLUMNA}",</p>',
                            '<p> name: "{COLUMNA}",</p>',
                            '<p> width: 240,</p>',
                         ...
                         '</tpl>',
                     '<tpl case="DATE">',
                        '<p> me.dat_{[values.COLUMNA.toLowerCase()]} = Ext.create("App.Config.Componente.DateFieldBase", &#123; </p>',
...
//                    alert(COMENTARIO.search('lista'));
    ...
    GenerarSpAlta : function(records, operations, successful){
        ...
        var TplDeclarar = new Ext.XTemplate(
                '<tpl for=".">',
                '<tpl for="data">',
                       '<p> p_{[values.COLUMNA.toLowerCase()]}  SINDICATO.{TABLA}.{COLUMNA}%type,</p>',
                    '</tpl>',
                   '</tpl>');
        var TplDefinirVariables = new Ext.XTemplate(
            ...
            '<tpl for=".">',
                '<tpl for="data">',
                    '<tpl if="ID_TABLA == \'1\'">',
                        '<p> v_{[values.COLUMNA.toLowerCase()]}  SINDICATO.{TABLA}.{COLUMNA}%type;</p>',
                    '</tpl>',
                '</tpl>',
             '</tpl>',
            '<p>v_cnt NUMBER:=0;</p>',
            '<p>v_res VARCHAR2(100):=\'0\';</p>',
            '<p>v_errC SD_AUX_LOG_ERRORES.cod_error%type;</p>',
            '<p>v_errD SD_AUX_LOG_ERRORES.desc_error%type;</p>',
            '<p>v_id_log SINDICATO.sd_aux_log_errores.id_log%type;</p>',
            '<p>BEGIN</p>'
        );
        var TplInsertSql = new Ext.XTemplate(
            '<p>  IF v_res=\'0\' THEN</p>',
            '<p> INSERT INTO SINDICATO.'+me.tabla+' (',
            '<tpl for=".">',
                '<tpl for="data">',
        ...
        var mainTpl = new Ext.XTemplate(
            '<p> CREATE OR REPLACE PROCEDURE SINDICATO.P_SD_ALTA_'+me.tabla+'(</p>',
            ...
            '<p> p_grabar_error_bd(v_errC,v_errD,\'P_SD_ALTA_'+me.tabla+'\',\'P_SD_ALTA_'+me.tabla+'\',\'-\',\'-\',v_id_log);</p>',
    ...
    GenerarSpModif : function(records, operations, successful){
        ...
        var TplDefinirVariablesMod = new Ext.XTemplate(
            ...
            '<p>v_errD SD_AUX_LOG_ERRORES.desc_error%type;</p>',
            '<p>v_id_log SINDICATO.sd_aux_log_errores.id_log%type;</p>',
            '<p>v_rg SINDICATO.'+me.tabla+'%rowtype;</p>',
        ...
        var mainModifTpl = new Ext.XTemplate(
            '<p> CREATE OR REPLACE PROCEDURE SINDICATO.P_EE_MODIF_'+me.tabla+'(</p>',
            ...
            '<p> v_errC:=substr(sqlcode,1,20);</p>',
            '<p>v_errD:=substr(sqlerrm,1,200);</p>',
            '<p> p_grabar_error_bd(v_errC,v_errD,\'P_EE_ALTA_'+me.tabla+'\',\'P_EE_ALTA_'+me.tabla+'\',\'-\',\'-\',v_id_log);</p>',
            '<p>  p_res :=\'ERROR. Avise a TI. LOG generado #\' || v_id_log;</p>',
            '<p> END;</p>',
            {
                esLista: function(COMENTARIO) {
