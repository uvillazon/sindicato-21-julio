...
/* Formatted on 10/04/2016 23:56:49 (QP5 v5.136.908.31019) */
...
(
  ...
  ID_SOCIO_MOVIL  NUMBER(7),
  FECHA_INICIO    DATE                          NOT NULL,
  FECHA_FIN       DATE                          NOT NULL,
  OBSERVACION     VARCHAR2(50 BYTE)             NOT NULL,
  FECHA_REG       DATE,
...
DROP PROCEDURE SINDICATO_132.P_SD_GUARDAR_PERMISOS;

CREATE OR REPLACE PROCEDURE SINDICATO_132.P_SD_GUARDAR_PERMISOS(
p_id_permiso SINDICATO_132.SD_PERMISOS.ID_PERMISO%type, 
p_id_socio_movil SINDICATO_132.SD_PERMISOS.ID_SOCIO_MOVIL%type, 
p_fecha_inicio SINDICATO_132.SD_PERMISOS.FECHA_INICIO%type, 
p_fecha_fin SINDICATO_132.SD_PERMISOS.FECHA_FIN%type, 
p_observacion SINDICATO_132.SD_PERMISOS.OBSERVACION%type, 
p_usr VARCHAR2,
p_res OUT VARCHAR2
)
IS
...
v_id_log SINDICATO_132.sd_aux_log_errores.id_log%type;
BEGIN
IF p_id_permiso  IS NULL OR p_id_socio_movil IS NULL OR p_fecha_inicio IS NULL OR p_fecha_fin IS NULL OR p_observacion IS NULL 
THEN
v_res := 'Faltan parametros.';
END IF;
IF p_id_permiso =  0 THEN
     
    IF v_res='0' THEN
        v_id := Q_SD_PERMISOS.nextval;
        INSERT INTO SINDICATO_132.SD_PERMISOS   (ID_PERMISO ,ID_SOCIO_MOVIL ,FECHA_INICIO ,FECHA_FIN ,OBSERVACION , FECHA_REG, LOGIN_USR   )
        VALUES( v_id , p_id_socio_movil ,p_fecha_inicio ,p_fecha_fin , p_observacion,  sysdate , p_usr );
         v_res := v_id;
    END IF;
ELSE
       IF v_res='0' THEN
         UPDATE SD_PERMISOS SET
         ID_SOCIO_MOVIL =  p_id_socio_movil ,  FECHA_INICIO = p_fecha_inicio , FECHA_FIN = p_fecha_fin ,OBSERVACION = p_observacion  ,  LOGIN_USR  = p_usr  
...
v_errC:=substr(sqlcode,1,20);
v_errD:=substr(sqlerrm,1,200);
p_grabar_error_bd(v_errC,v_errD,'P_SD_GUARDAR_PERMISOS','P_SD_GUARDAR_PERMISOS','-','-',v_id_log);
p_res :='ERROR. Avise a TI. LOG generado #' || v_id_log;
END;
/
