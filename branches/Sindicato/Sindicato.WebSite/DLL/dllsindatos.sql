...
/* Formatted on 29/04/2016 1:05:47 (QP5 v5.136.908.31019) */
...
BEGIN
  ...
  WHILE v_fecha < v_fecha_hasta LOOP
   ...
   END IF;
    ...
    IF v_cantidad_hojas_oblig > v_total_hoja THEN
...
IF v_res =  '0' THEN
        ...
        IF v_cnt > 0 THEN
                  ...
                  SELECT FECHA_COMPRA INTO v_fecha FROM SD_HOJAS_CONTROL WHERE ID_HOJA = p_id_hoja ;
...
IF v_res =  '0' THEN
        ...
        IF v_cnt > 0 THEN
                  SELECT FECHA_COMPRA INTO v_fecha FROM SD_REGULARIZACIONES WHERE ID_REGULACION  = p_id_regulacion ;
...
IF v_res =  '0' THEN
        ...
        SELECT COUNT(*) INTO v_cnt_numero FROM SD_HOJAS_CONTROL WHERE ID_SOCIO_MOVIL = p_ID_SOCIO_MOVIL AND TO_CHAR(FECHA_COMPRA,'MM/YYYY' ) = TO_CHAR(p_fecha_compra,'MM/YYYY');
        IF v_cnt_numero > 0 THEN
            SELECT MAX(NRO_HOJA )  + 1 INTO v_numero FROM SD_HOJAS_CONTROL WHERE ID_SOCIO_MOVIL = p_ID_SOCIO_MOVIL AND TO_CHAR(FECHA_COMPRA,'MM/YYYY' ) = TO_CHAR(p_fecha_compra,'MM/YYYY');
        ...
        WHILE v_cnt < p_cantidad LOOP
            ...
            (
            SELECT Q_SD_DETALLES_HOJAS_CONTROL.nextval , v_id_hoja ,OB.ID_CAJA ,OB.OBLIGACION,OB.IMPORTE_DEFECTO , p_usr , sysdate  from SD_SOC_MOV_OBLIG soc , SD_OBLIGACIONES_HOJA  ob where SOC.ID_OBLIGACION =  OB.ID_OBLIGACION AND id_socio_movil = p_ID_SOCIO_MOVIL
            );
            INSERT INTO SD_KARDEX_EFECTIVO (ID_KARDEX ,ID_CAJA ,ID_OPERACION ,OPERACION ,FECHA, DETALLE ,INGRESO ,EGRESO, SALDO, LOGIN, FECHA_REG )
            (
