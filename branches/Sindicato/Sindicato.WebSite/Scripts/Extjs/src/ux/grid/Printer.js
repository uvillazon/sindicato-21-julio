...
Ext.define("Ext.ux.grid.Printer", {
    ...
    requires: ['Ext.XTemplate', 'Ext.ux.Exporter'],

    statics: {

        export: function (grid, load) {
            var me = this;
            if (load === undefined) {
                var gridcopy = grid.cloneConfig();
                ...
                store2.proxy.timeout = 600000;
                store2.load({limit: grid.store.getTotalCount(), start: 0, page: 1});
                store2.on('load', function (store, records, options) {
                    gridcopy.store = store;
                    var filename = 'exportarDatos';
                    var data = Ext.ux.Exporter.exportGrid(store, gridcopy, 'excel', filename);
                    window.open('data:application/vnd.ms-excel,' + encodeURIComponent(data));
                });
            }
            else{
                var filename = 'exportarDatos';
                var data = Ext.ux.Exporter.exportGrid(grid.getStore(), grid, 'excel', filename);
                window.open('data:application/vnd.ms-excel,' + encodeURIComponent(data));
            }
        },
        print: function (grid, featureId, load) {
            var me = this;
            if (load === undefined) {
                me.mainTitle = grid.title;
                var gridcopy = grid.cloneConfig();
                var store2 = new Ext.data.Store({
                    proxy: grid.store.proxy,
                    reader: grid.store.reader,
                ...
                store2.on('load', function (store, records, options) {
                    gridcopy.store = store;
                    me.printLoad(gridcopy, featureId);
                });
            }
            else {
                me.printLoad(grid, featureId);
            }
        },
        /**
         ...
         */
        printLoad: function (grid, featureId) {
                ...
                '</html>',
                {
                    ...
                    renderColumn: function (column, value, rcd, col) {
                        var meta = {
                            ...
                            'tdCls': '',
                            'unselectableAttr': 'unselectable="on"',
                            'value': (value == undefined) ? grid.store.indexOf(rcd).toString() + 1 : value
                        };
                    ...
                    getHtml: function (value, meta) {
                        ...
                        // if(meta.column.xtype ==="rownumberer"){
                        //     value = meta.value;
                        // }
                        var html = '<td ';
                        var tdClasses = '';
